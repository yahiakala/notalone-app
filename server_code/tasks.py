...
def delete_user_bk(tenant_id, user, user_email):
    ...
    user_del_tenant, user_del_usermap, user_del_permissions = validate_user(
        tenant_id, user_del
    )

    if "delete_members" in user_del_permissions and "delete_admin" not in permissions:
        raise Exception(
            "Only users with the delete_admin permission can delete this user."
        )
...
def edit_member(tenant_id, new_dict):
    ...
    if "edit_members" in permissions:
        roles = app_tables.roles.search(
            tenant=tenant, name=q.any_of(*new_dict["roles"])
        )
...
def save_user_notes(tenant_id, user_email, new_note):
    ...
    anvil.server.launch_background_task(
        "save_user_notes_bk", tenant_id, user, user_email, new_note
    )
...
def remove_role_from_member(tenant_id, role_name, member_email):
    ...
    user = anvil.users.get_user(allow_remembered=True)
    anvil.server.launch_background_task(
        "remove_role_from_member_bk", tenant_id, user, role_name, member_email
    )
...
def add_role(tenant_id, role_name, role_perms):
    ...
    if not app_tables.roles.get(tenant=tenant, name=role_name):
        ...
        return [
            role_row_to_dict(role) for role in app_tables.roles.search(tenant=tenant)
        ]
...
def update_role(tenant_id, role_name, new_role_dict):
    ...
    role["last_update"] = dt.date.today()
    perm_rows = app_tables.permissions.search(
        name=q.any_of(*new_role_dict["permissions"])
    )
...
def search_users_by_text(tenant_id, search_string):
    ...
    users = app_tables.users.search(
        q.fetch_only("email"), q.any_of(email=q.ilike("%" + search_string + "%"))
    )
...
def get_member_data(tenant_id, email):
    ...
    member = app_tables.users.get(q.fetch_only("email"), email=email)
...
def update_tenant_data(tenant_id, new_dict):
    ...
    for secret_key in secrets:
        tenant[secret_key] = anvil.secrets.encrypt_with_key(
            "encryption_key", new_dict[secret_key]
        )
