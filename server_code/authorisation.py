...
config = {"get_roles": itemgetter("roles"), "tenanted": True}
...
def _get_roles_from_table(table_name, user, tenant=None):
    if tenant:
        return getattr(app_tables, table_name).get(user=user, tenant=tenant)["roles"]
    else:
...
def has_permission(permissions):
    ...
    try:
        user_permissions = set(
            permission["name"]
            for role in config["get_roles"](user, tenant)
