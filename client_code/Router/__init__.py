...
@routing.template(
    path="app", priority=5, condition=lambda: Global.get_s("tenant") is not None
)
class Router(RouterTemplate):
    ...
    def set_account_state(self, user):
        ...
        if user:
            ...
            self.link_volunteers.visible = "see_forum" in self.permissions
    ...
    def link_forum_nav_click(self, **event_args):
        ...
        if "see_forum" not in Global.permissions:
            routing.alert(
                "Please make sure your membership is in good standing before accessing the forum."
            )
        elif not Global.usermap["consent_check"]:
            routing.alert(
                "Please make sure you have consented to the code of conduct in the profile page and saved before accessing the forum."
            )
        else:
    ...
    def link_help_click(self, **event_args):
        """This method is called when the link is clicked"""
        anvil.alert("For help, contact " + Global.tenant["email"])
    ...
    def on_navigation(self, url_hash, url_pattern, url_dict, unload_form):
        for link in self.cp_sidebar.get_components():
            if isinstance(link, anvil.Link):  # noqa
