...
class ProfileComponent(ProfileComponentTemplate):
    def __init__(self, **properties):
        ...
        if self.user['auth_screenings']:
            if self.user['screening_slots']:
                # self.screening_slots = self.user['screening_slots']
                # self.screening_slots = self.default_screening_slots()
                pass
            else:
                # self.screening_slots = self.default_screening_slots()
                self.user['screening_slots'] = self.default_screening_slots()
                
            from .DaySelector import DaySelector
            for slot in self.user['screening_slots']:
    ...
    def default_screening_slots(self, **event_args):
        return [
            {
                "day": 0,
                "end": 17,
