...
@routing.redirect(
    path="app",
    priority=18,
    condition=lambda: Global.get_s("tenant") is None and Global.user is not None,
)
...
@routing.redirect(
    path="app",
    priority=17,
    condition=lambda: Global.get_s("tenant") is not None
    and "book_interview" in Global.permissions
    and "see_profile" not in Global.permissions,
)
