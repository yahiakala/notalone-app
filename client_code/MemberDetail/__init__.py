...
class MemberDetail(MemberDetailTemplate):
    ...
    def load_data(self):
        ...
        if self.member["payment_expiry"] and self.member["payment_status"]:
            ...
            self.lbl_next_payment_value.text = self.member["payment_expiry"].strftime(
                "%Y-%m-%d"
            )
            if self.member["payment_status"] != "ACTIVE":
        ...
        if (
            self.member["payment_status"] != "ACTIVE"
            and "profile" in routing.get_url_pattern()
        ):
    ...
    def btn_save_click(self, **event_args):
        ...
        with anvil.server.no_loading_indicator:
            ...
            if (
                "memberdetail" in routing.get_url_pattern()
                and "edit_members" in Global.permissions
            ):
    ...
    def pay_click(self, item, **event_args):
        ...
        with anvil.server.no_loading_indicator:
            self.member, self.payment_url = anvil.server.call(
                "create_sub", Global.tenant_id, item["id"]
            )
            self.btn_save_click()
            routing.clear_cache()
    ...
    def btn_reject_applicant_click(self, **event_args):
        """This method is called when the button is clicked"""
        self.member = anvil.server.call(
            "reject_applicant", Global.tenant_id, self.email
        )
        self.populate_role_list()
        routing.clear_cache()
    ...
    def btn_accept_applicant_click(self, **event_args):
        """This method is called when the button is clicked"""
        self.member = anvil.server.call(
            "accept_applicant", Global.tenant_id, self.email
        )
        self.populate_role_list()
        routing.clear_cache()
        self.btn_accept_applicant.visible = False
...
Instead of deleting, you can remove the member's access by clearing all their roles in the role selection above.
...
Do you want to proceed with deletion?""",
    ...
    def populate_role_list(self):
        ...
        self.msc_roles.items = [
            {"key": i["name"], "value": i["name"], "description": i["name"]}
            for i in Global.roles
        ]
        self.msc_roles.selected = [
            {"key": i, "value": i, "description": i} for i in self.member["roles"]
        ]
    ...
    def btn_refresh_click(self, **event_args):
        ...
        with anvil.server.no_loading_indicator:
            try:
                self.member = anvil.server.call(
                    "refresh_subscription_data", Global.tenant_id, self.email
                )
                routing.reload_page(hard=True)
            except Exception as e:
                if "No active subscription found" in str(e):
                    anvil.alert("No active subscription found.")
                else:
                    raise
